...
- 系统自动备份已关闭；跨设备迁移应使用应用内导出的 JSON 备份文件。

### 提醒

- 可以为课程设置课前提醒。
- 使用 Android 系统的定时任务调度提醒，不需要应用持续运行在前台。
- Android 12 及以上可授予“闹钟和提醒”特殊访问以获得精确提醒；未授予时会自动使用可能延迟的兼容调度。
...
- 自定义教务入口只接受 HTTPS。江苏大学登录从 HTTPS 统一认证开始，但其课表接口仍为 HTTP；应用会限制明文流量到该教务域名并在页面中提示残余的会话窃取风险。
